...
@WebServlet("/bookquery")   // Configure the request URL for this servlet (Tomcat 7/Servlet 3.0 upwards)
public class bookquery extends HttpServlet {
   ...
   public void doPost(HttpServletRequest request, HttpServletResponse response)
      ...
      try (
		...
		String sqlStr = "SELECT * FROM books WHERE qty > 0 ORDER BY author ASC, title ASC";
...
// For each row in ResultSet, print one checkbox inside the <form>
        ...
        out.println("<form id='contact-form' method='post' action='eshopquerytitle'>");
			out.println("<p>Choose books:</p>");
			out.println("<p>");
			while(rset.next())
            {
             	out.println("<input type='checkbox' name='title' class='frm_inpt' value=" + "'" + rset.getString("title") + "' />");
